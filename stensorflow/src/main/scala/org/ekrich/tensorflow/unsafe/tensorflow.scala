 ...
 *     numpy array.
 *   - We do not provide TF_OK since it is not strictly necessary and we are not
...
object tensorflow {
  ...
  type TF_AttrMetadata = CStruct4[
    ...
    TF_AttrType,
    /**
     * Total size the attribute value. The units of total_size depend on is_list
     * and type.
     *   - (1) If type == TF_ATTR_STRING and is_list == 0 then total_size is the
     *     byte size of the string valued attribute.
     *   - (2) If type == TF_ATTR_STRING and is_list == 1 then total_size is the
     *     cumulative byte size of all the strings in the list.
     *   - (3) If type == TF_ATTR_SHAPE and is_list == 0 then total_size is the
     *     number of dimensions of the shape valued attribute, or -1 if its rank
     *     is unknown.
   ...
   * to its length. Returns the number of bytes starting at `src` consumed while
   ...
   * tensor), and TF_AddInputList() for the second input (since it takes a list,
   * even if you were to pass a list with a single tensor), as in:
   * {{{
  ...
  /**
   * TF_Operation functions. Operations are immutable once created, so these are
   * all query functions.
   */
   ...
   * to an array of length `max_control_inputs` (ideally set to
   ...
   * `max_control_outputs` (ideally set to TF_OperationNumControlOutputs(oper)).
   * Beware that a concurrent modification of the graph can increase the number
   * of control outputs. Returns the number of control outputs (should match
   ...
   *
   * Allocates a new TF_Tensor which the caller is expected to take ownership of
   * (and can deallocate using TF_DeleteTensor).
   */
   ...
   * where a device was not already explicitly specified. `device` is copied and
   ...
   * operation names will be modified if their name already exists in the graph.
   * If false, conflicting names will be treated as an error. Note that this
   * option has no effect if a prefix is set, since the prefix will guarantee
   ...
   * replaced with `dst`. `src_name` refers to a node in the graph to be
   * imported, `dst` references an operation already existing in the graph being
   * imported into. `src_name` is copied and has no lifetime requirements.
   */
   ...
   * parameter of TF_GraphImportGraphDef(). If the output is remapped via an
   * input mapping, the corresponding existing tensor in `graph` will be
   ...
   * output parameter of TF_GraphImportGraphDef(). `oper_name` is copied and has
   ...
   * operation using the function's name. Any changes to `func`/`grad`
   ...
   * have a gradient. If `func` already has a gradient different from `grad`, an
   * error is returned.
   *
   * `func` must not be null. If `grad` is null and `func` is not in `g`, `func`
   * is added without a gradient. If `grad` is null and `func` is in `g`,
   * TF_GraphCopyFunction is a noop. `grad` must have appropriate signature as
   ...
   * error, returns 0, sets status to the encountered error, and the contents of
   * funcs will be undefined.
   */
   ...
   *     other function and operation names (at least those registered in graphs
   *     where this function will be used).
   *
   * append_hash_to_fn_name
   *   - Must be 0 or 1. If set to 1, the actual name of the function will be
   *     `fn_name` appended with '_<hash_of_this_function's_definition>'. If set
   *     to 0, the function's name will be `fn_name`.
   *
   * num_opers
   ...
   *     `ninputs` is zero (the function takes no inputs), `inputs` can be null.
   ...
   *     all letters to lowercase and non-alphanumeric characters to '_' to make
   *     resulting names match the "[a-z][a-z0-9_]*" pattern for operation
   ...
   * the corresponding function's output will equal to this input, instead of
   ...
   *     support reference types inside function's body to support legacy code.
   *     Do not use them in new code.
   *   - Every node in the function's body must have all of its inputs
   *     (including control inputs). In other words, for every node in the body,
   *     each input must be either listed in `inputs` or must come from another
   *     node in the body. In particular, it is an error to have a control edge
   *     going from a node outside of the body into a node in the body. This
   *     applies to control edges going from nodes referenced in `inputs` to
   *     nodes in the body when the former nodes are not in the body
   *     (automatically skipped or not included in explicitly specified body).
   *
   ...
   * Does not take ownership of any input parameters.
   *
   ...
   * (inputs[0,ninputs-1] with corresponding values in
  ...
  /**
   * Set up the graph with the intended feeds (inputs) and fetches (outputs) for
   ...
   * needed.
   *
   * On failure, out_status contains a tensorflow::Status with an error message.
   * *handle is set to nullptr.
   */
   ...
   * return value will be a pointer to a null terminated string. The caller must
   * not modify or delete the string. It will be deallocated upon a call to
   * TF_DeleteDeviceList.
   *
   ...
   *
   * The server will not serve any requests until TF_ServerStart is invoked. The
   * server will stop serving requests once TF_ServerStop or TF_DeleteServer is
   * invoked.
   */
  def TF_NewServer(
      proto: Ptr[Byte],
